# Structural Equation Model: Selecting direct effects in DAG (Step 1)
...
sex_id = as.numeric(args[1])
cat = c("Female", "Male")[sex_id + 1]
print(paste0("Sex: ",cat))

instance = as.numeric(args[2])
...
outcome = outcome[rownames(outcome) %in% common_ids, grepl(instance, colnames(outcome))]
...
# Stability-enhanced penalised regression (LASSO, gaussian, n_cat=3)
stab = VariableSelection(cbind(Z,X), Y, family = "gaussian",
                         n_cat=3, pi_list = seq(0.5, 0.99, by = 0.01),
                         K = 100, tau = 0.5,
                         penalty.factor = c(rep(0,ncol(Z)),rep(1,ncol(X))),
                         n_cores = n_cores)

pdf(paste0("../Figures/1-CalibrationPlot_",cat,"_Instance",instance,".pdf"))
par(mar = c(7,5,7,6))
CalibrationPlot(stab)
dev.off()

saveRDS(stab, paste0("../Results/1-stab_",cat,"_Instance",instance,".rds"))
